...
try:
    browser.find_element_by_id(
        "tw-source-text-ta").send_keys(" ".join(sys.argv[1:]).capitalize())
except IndexError:
    browser.find_element_by_id("tw-source-text-ta").send_keys(
        "When launching the program enter something for it to work")
WebDriverWait(browser, 1000000).until(
    EC.element_to_be_clickable((By.ID, 'tw-src-spkr-button'))).click()
